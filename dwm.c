...
static char stext[1024]; //with ^^
static char mstext[1024]; //with \x
static char ctext[1024]; //clean text
static char rawstext[1024]; //initial text
...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
		...
		if (i < LENGTH(tags)) {
			...
			click = ClkLtSymbol;
		else if (ev->x > (x = selmon->ww - TEXTW(ctext) + lrpad)) {
			click = ClkStatusText;
			char *text = mstext;
			int i = -1;
			char ch;
...
void
copyvalidcharsC(char *text, char *rawtext)
{
	int i = -1, j = 0;

	while(rawtext[++i]) {
        if ((unsigned char)rawtext[i] == '^') {
            while((unsigned char)rawtext[++i] != '^') ;
        }
        else if ((unsigned char)rawtext[i] >= ' ') {
            text[j++] = rawtext[i];
        }
    }
    text[j] = '\0';
...
void
copyvalidcharsM(char *text, char *rawtext)
{
	int i = -1, j = 0;
...
updatestatus(void)
{
    ...
    else
    {
        ...
        copyvalidcharsM(mstext, rawstext);
        copyvalidcharsC(ctext, rawstext);
    }
    drawbar(selmon);
}
